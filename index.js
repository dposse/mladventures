...
class PolicyNetwork {
   ...
   *
   * @param {BoardPlayer} cartPoleSystem The cart-pole system object to use during
   *   training.
  ...
  async train(
    ...
    for (let i = 0; i < numGames; ++i) {
      ...
      // of every game.
      //no longer doing the above, initialize with empty board
      cartPoleSystem.setInitialState();
      const gameRewards = [];
      const gameGradients = [];
  ...
  getGradientsAndSaveActions(inputTensor) {
    const f = () => tf.tidy(() => {
      ...
      // console.log(this.getLogitsAndActions(inputTensor));
